...
class PaymentServiceImpl(
    ...
    override suspend fun registerTransaction(
        ...
        if (paymentConfig.mockMode) {
            println("Payment mock mode enabled – skipping Przelewy24 API")
            ...
            return PaymentRegistration(token = "mock-$sessionId", sessionId = sessionId)
        }
    ...
    override fun paymentRedirectUrl(token: String): String =
        if (paymentConfig.mockMode) paymentConfig.urlReturn
        else "${paymentConfig.redirectBaseUrl}$token"
    ...
    override suspend fun handlePaymentNotification(notification: P24Notification) {
        ...
        val completed = completePendingReservation(pending, notification.orderId)
            ?: throw IllegalStateException("Nie udało się utworzyć rezerwacji")
        if (completed.status == PaymentSessionStatus.FAILED) {
            throw IllegalStateException(completed.errorMessage ?: "Nie udało się utworzyć rezerwacji")
        }
    }

    private suspend fun completePendingReservation(
        pending: PendingPaymentTable,
        orderId: Long?,
    ): PendingPaymentTable? {
        if (pending.status == PaymentSessionStatus.COMPLETED) {
            return pending
        ...
        return try {
            val reservationDto = pending.reservation.toDto().copy(
    ...
    override suspend fun getPaymentStatus(sessionId: String): PaymentStatusResponse {
        var pending = pendingPaymentRepo.findBySessionId(sessionId)
        ...
        if (paymentConfig.mockMode && pending.status == PaymentSessionStatus.PENDING) {
            pending = completePendingReservation(pending, orderId = null) ?: pending
        }
